...
import "./StaffPage.css"; // Đảm bảo đã import file CSS
...
export default function StaffPage() {
    ...
    const renderSidebar = () => (
        <aside className="sidebar">
            <div className="logo">
                <img src="/logo.png" alt="Logo" className="logo" /> {/* Đảm bảo có logo.png trong public folder */}
            </div>
            <nav>
                <ul className="sidebar-menu"> {/* Đã thêm class sidebar-menu */}
                    {sidebarMenu.map((item) => (
                        <li key={item.key}>
                            ...
                            >
                                <span className="menu-icon">{item.icon}</span> {/* Đã thêm class menu-icon */}
                                {item.label}
                            </a>
                        </li>
                    ))}
    ...
    // Render nội dung chính dựa trên activeMenu
    const renderMainContent = () => {
        switch (activeMenu) {
            case "dashboard":
                return <div>Nội dung tổng quan của Staff...</div>; // Placeholder
            case "consultations":
                return <div>Nội dung quản lý đơn tư vấn của Staff...</div>; // Placeholder
            case "tests":
                return renderTestsTable();
            case "samples":
                return <div>Nội dung quản lý mẫu xét nghiệm của Staff...</div>; // Placeholder
            case "results":
                return <div>Nội dung quản lý kết quả xét nghiệm của Staff...</div>; // Placeholder
            case "profile":
                return <div>Nội dung thông tin cá nhân của Staff...</div>; // Placeholder
            default:
                return renderTestsTable(); // Mặc định hiển thị bảng đơn xét nghiệm
        }
    };
